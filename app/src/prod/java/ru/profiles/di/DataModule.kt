...
class DataModule {
    ...
    @Provides
    @Singleton
    fun provideAuthRepository(authDao: AuthModelDao, authApi: AuthApi): AuthRepositoryOps {
        return AuthRepository.getInstance(authDao, authApi)
    }

    @Provides
    @Singleton
    fun provideUserRepository(userDao: UserModelDao, authApi: AuthApi): UserRepositoryOps{
